...
class MetamaterialModel:
    def __init__(self, h_total=0.001, ratio=0.5, T=25.0, thetaL=0.0, 
                 limFlag='piecewise', debugFlag=False,
                 a=0.018):
...
def analyze_parameter_space(T_range, h_range, ratio_range, thetaL_range):
    q = np.radians(np.arange(-45.0,45.5,0.5))# Could increase the resolution here...
...
def find_phase_boundaries(T_range, h_range, ratio_range, thetaL_range,
    ...
    print(max(T_range))
    print(max(boundaryT[:,0]))
    print(max(1000*h_range))
    print(max(boundaryh[:,0]))
    print(max(np.degrees(thetaL_range)))
    print(max(boundarytheta[:,0]))
    #print(boundarytheta[0])
